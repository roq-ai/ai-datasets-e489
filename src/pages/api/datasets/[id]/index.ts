...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getDatasetById() {
    const data = await prisma.dataset.findFirst(convertQueryToPrismaUtil(req.query, 'dataset'));
    return res.status(200).json(data);
  }

  async function updateDatasetById() {
  ...
  async function deleteDatasetById() {
    const data = await prisma.dataset.delete({
      where: { id: req.query.id as string },
    });
    return res.status(200).json(data);
  }
