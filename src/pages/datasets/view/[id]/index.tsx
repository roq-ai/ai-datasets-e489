...
import { getDatasetById } from 'apiSdk/datasets';
import { DatasetInterface } from 'interfaces/dataset';
import { ContributionListPage } from 'pages/contributions';
import { ReviewListPage } from 'pages/reviews';

function DatasetViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<DatasetInterface>(
    () => (id ? `/datasets/${id}` : null),
    () =>
      getDatasetById(id, {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('dataset', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/datasets/edit/${id}`} passHref legacyBehavior>
                    <Button
