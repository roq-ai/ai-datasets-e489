...
import { getDatasets, deleteDatasetById } from 'apiSdk/datasets';
import { DatasetInterface } from 'interfaces/dataset';

type ColumnType = ColumnDef<DatasetInterface, unknown>;
...
export function DatasetListPage(props: DatasetListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getDatasets({
        relations: ['organization', 'user', 'contribution.count', 'review.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<DatasetInterface>>(
    () => `/datasets?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: DatasetInterface) => {
    if (hasAccess('dataset', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/datasets/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('contribution', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'contribution',
          header: 'Contribution',
          accessorKey: 'contribution',
          cell: ({ row: { original: record } }: any) => record?._count?.contribution,
        }
      : null,
...

    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/datasets/view/${record.id}`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              mr={2}
              padding="0rem 8px"
          ...
          {hasAccess('dataset', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/datasets/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('dataset', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="dataset" />
        </Flex>

        {hasAccess('dataset', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/datasets/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
